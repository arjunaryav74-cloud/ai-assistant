...
// Per-OS system-control guidance comes from the platform adapter
// (electron/platform/mac.ts vs windows.ts — shared lines in shared-prompt.ts).
// process.platform is constant per process, so resolving it once at module
// load keeps the prompt string stable and cacheable.
const PLATFORM_CONTROL_BLOCK = currentPlatform().controlPromptBlock;
