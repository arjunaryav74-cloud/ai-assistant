...
async function updateMemoryAccess(ids: string[]): Promise<void> {
  if (ids.length === 0) return;
  try {
    await getSupabase().rpc("increment_memory_access", { p_memory_ids: ids });
  } catch (err: unknown) {
...
function buildContextHeader(plan: RetrievalPlan): string {
